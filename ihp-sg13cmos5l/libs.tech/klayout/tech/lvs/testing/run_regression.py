...
Supported: MOS, RES, DIODE, ESD (diodevdd/vss, nmoscl), TAP, BJT (pnpMPA),
           IND (inductor2, TopMetal1 winding), CAP (cap_cmomi, moscaps),
           CAP_CMOMF (cap_cmomf)
...
def build_tests_dataframe(unit_test_cases_dir, target_device_group):
    ...
    """
    # Use glob.glob with ** to follow symlinks (Path.rglob doesn't follow symlinks)
    layout_pattern = os.path.join(unit_test_cases_dir, "**", f"*.{SUPPORTED_TC_EXT}")
    all_unit_test_cases_layout = sorted(
        [Path(p) for p in glob.glob(layout_pattern, recursive=True)]
    )
    logging.info(
    ...
    netlist_pattern = os.path.join(unit_test_cases_dir, "**", f"*.{SUPPORTED_SPICE_EXT}")
    all_unit_test_cases_netlist = sorted(
        [Path(p) for p in glob.glob(netlist_pattern, recursive=True)]
    )
    logging.info(
...
def run_regression(lvs_dir, output_path, target_device_group, cpu_count):
    ...
    # Filter to only CMOS5L-compatible device groups
    devices_df = devices_df[devices_df["device_group"].isin(allowed_device_groups)]

    # Exclude specific devices not in CMOS5L
    devices_df = devices_df[~devices_df["device_name"].isin(excluded_devices)]
    logging.info(
        "Total number of CMOS5L-compatible devices found: {}".format(len(devices_df))
    )
...
if __name__ == "__main__":
    ...
    # Excluded: RFMOS. CAP is enabled for cap_cmomi (other CAP-group devices
    # are filtered by excluded_devices); CAP_CMOMF is the MoM fringe capacitor,
    # see the note on allowed_device_groups above.
